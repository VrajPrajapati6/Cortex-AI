...
Before training any machine learning model, Cortex will generate datasets from the telemetry stored in PostgreSQL.

The dataset generation process will export historical operational data and organize it into a structured format suitable for preprocessing and model training.
...
```
datasets/
├── raw/          # Raw exported telemetry
└── processed/    # Cleaned datasets ready for training
```

### Dataset Generation Script

The `scripts/generate_dataset.py` script will be responsible for exporting telemetry data from PostgreSQL and creating the initial raw datasets. The implementation will be added in future development phases.
